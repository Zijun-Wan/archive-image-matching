# ========= LOAD SIDE (Server) =========
# Load the saved model and return a ready-to-query VocabTreeDB instance.
# Usage on server:
...
def load_vt_model(in_dir):
    ...
    def import_tree_dict(tree_dict, VocabTreeClass, VocabNodeClass):
        k = int(tree_dict.get("k", 10)); L = int(tree_dict.get("L", 6))
        tree = VocabTreeClass(k=k, L=L)
        nodes_by_id = {}
        for d in tree_dict["nodes"]:
            n = VocabNodeClass(
                centroid=(None if d["centroid"] is None else np.ascontiguousarray(d["centroid"], dtype=np.float32)),
        ...
        for d in tree_dict["nodes"]:
            n = nodes_by_id[int(d["node_id"])]
            n.children = [nodes_by_id[int(cid)] for cid in d["children_ids"]]
        tree.root = nodes_by_id[int(tree_dict["root_id"])]
        tree.leaf_nodes = [nodes_by_id[int(d["node_id"])] for d in tree_dict["nodes"] if d["is_leaf"]]
        tree._next_id = (max(nodes_by_id.keys()) + 1) if nodes_by_id else 0
        lim = tree_dict.get("leaf_id_map")
        tree.leaf_id_map = ({int(k): int(v) for k, v in lim.items()}
                            if lim else {leaf.node_id: i for i, leaf in enumerate(tree.leaf_nodes)})
        return tree

    def import_index_dict(idx_dict, InvertedIndexClass):
        idx = InvertedIndexClass(num_leaves=int(idx_dict["num_leaves"]))
        idx.doc_tf   = {int(d): {int(w): int(tf) for w, tf in tfmap.items()}
                        for d, tfmap in idx_dict["doc_tf"].items()}
        idx.idf      = np.ascontiguousarray(idx_dict["idf"], dtype=np.float32)
        idx.doc_norm = {int(d): float(v) for d, v in idx_dict["doc_norm"].items()}
        idx.N        = int(idx_dict.get("N", len(idx.doc_tf)))
        idx.ext2int  = {k: int(v) for k, v in idx_dict.get("ext2int", {}).items()}
        idx.int2ext  = list(idx_dict.get("int2ext", []))
        idx.stop_leaves = set(idx_dict.get("stop_leaves", set()))
        idx._build_postings()
        return idx

    def _tuple_to_kp(t):
        # supports (x,y), (x,y,size), or (x,y,size,angle,response,octave,class_id)
        if isinstance(t, (list, tuple)):
            if len(t) == 2:
                x, y = t
                return cv2.KeyPoint(x=float(x), y=float(y), _size=1.0)
            if len(t) == 3:
                x, y, s = t
                return cv2.KeyPoint(x=float(x), y=float(y), _size=float(s))
            if len(t) >= 7:
                x, y, s, a, r, o, c = t[:7]
                return cv2.KeyPoint(x=float(x), y=float(y), _size=float(s),
                                    _angle=float(a), _response=float(r),
                                    _octave=int(o), _class_id=int(c))
        raise TypeError(f"Bad keypoint tuple: {t}")

    # load files
    tree_dict = _bytes_load_gz(os.path.join(in_dir, "tree.pkl.gz"))
    idx_dict  = _bytes_load_gz(os.path.join(in_dir, "index.pkl.gz"))

    tree  = import_tree_dict(tree_dict, VocabTree, VocabNode)
    index = import_index_dict(idx_dict, InvertedIndex)

    db = VocabTreeDB(k=int(tree_dict.get("k", 10)), L=int(tree_dict.get("L", 6)))
    db.tree  = tree
    db.index = index

    # meta: rebuild any tuple keypoints into cv2.KeyPoint
    meta_path = os.path.join(in_dir, "meta.pkl.gz")
    if os.path.exists(meta_path):
        meta = _bytes_load_gz(meta_path)
        im = meta.get("image_meta", {}) or {}
        for img_id, m in im.items():
            kps = m.get("kps", None)
            if isinstance(kps, list) and kps and not isinstance(kps[0], cv2.KeyPoint):
                try:
                    kps = [_tuple_to_kp(t) for t in kps]
                except Exception:
                    # if conversion fails, keep tuples; spatial patch will handle them
                    pass
            descs = m.get("descs", None)
